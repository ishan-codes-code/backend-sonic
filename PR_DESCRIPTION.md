...
Application startup happens in [main.ts](/e:/Codes/backends/sonic-backend/src/main.ts).
...
The root composition is defined in [app.module.ts](/e:/Codes/backends/sonic-backend/src/app.module.ts).
...
- Feature modules are `AuthModule`, `SongsModule`, `SongFilesModule`, `LibraryModule`, and `StreamModule`.
...
- `src/song-files/`
  - YouTube download and audio extraction pipeline.
- `src/r2/`
  - Cloudflare R2 upload and signed URL generation.
- `src/library/`
  - User library endpoints and ownership management.
- `src/stream/`
  - Playback URL retrieval for songs already owned by a user.
- `src/common/`
...
Job state is managed by [job.service.ts](/e:/Codes/backends/sonic-backend/src/songs/job.service.ts).
...
When a new song needs processing:
...
7. Mark the job `done` with `streamUrl`.

If any step fails, the job is marked `error`.
...
- `GET /library`
  - Lists songs owned by the current user.
- `POST /library/addSong`
  - Adds a song to the user library, downloading it first if necessary.

### Stream

- `GET /stream/:songId`
  - Returns a signed playback URL only if the user owns the song.
...
## Workflow: Add Song to Library

1. Client calls `POST /library/addSong`.
2. Existing song path:
   add ownership only.
3. Missing song path:
   download, upload, create DB row, then add ownership.
...
- Background processing currently uses an in-memory job map rather than Redis or a durable queue.
...
- The service is expected to run as a single instance when relying on in-memory jobs.
- Clients know to poll when `/songs/play` returns `{ type: "job", jobId }`.

## Known Issues and Limitations

- Job state is not durable.
  - Restarting the server clears all in-memory jobs.
  - Multi-instance deployments will not share job state.
...
- Move job state to Redis or another durable queue/store.
